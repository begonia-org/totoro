...
def tokenize(d, t, eng):
    ...
    d["content_tokens"] = tokenizer.tokenize(t)
    d["content_sm_ltks"] = tokenizer.fine_grained_tokenize(d["content_tokens"])
...
def add_positions(d, poss):
    ...
    for pn, left, right, top, bottom in poss:
        d["page_num"].append(int(pn + 1))
        d["top"].append(int(top))
...
def hierarchical_merge(bull, sections, depth):
    ...
    for ck in cks:
        if len(ck) == 1:
            n = encoder.num_tokens_from_string(
                re.sub(r"@@[0-9]+.*", "", ck[0]))
            if n + num[-1] < 218:
                res[-1].append(ck[0])
                num[-1] += n
