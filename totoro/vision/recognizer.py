...
from totoro.utils.logger import task_logger
...
class Recognizer(object):
    def __init__(self, label_list, task_name, model_dir=None):
        ...
        if ort.get_device() == "GPU":
            print("Using Recognizer GPU")
            options = ort.SessionOptions()
            options.enable_cpu_mem_arena = False
            self.ort_sess = ort.InferenceSession(
                model_file_path, options=options, providers=[
        ...
        while i + 1 < len(layouts):
            ...
            for b in boxes:
                ...
                if not notOverlapped(b, layouts[j]):
                    area_i_1 += Recognizer.overlapped_area(
                        b, layouts[j], False)
            ...
            for op_info in [
                {
                    'interp': 2, 'keep_ratio': False, 'target_size': [
                        800, 608], 'type': 'LinearResize'}, {
                    'is_scale': True, 'mean': [
                    0.485, 0.456, 0.406], 'std': [
                        0.229, 0.224, 0.225], 'type': 'StandardizeImage'}, {
                            'type': 'Permute'}, {
    ...
    def postprocess(self, boxes, inputs, thr):
        if "scale_factor" in self.input_names:
            ...
            for b in boxes:
                ...
                if clsid >= len(self.label_list):
                    task_logger.warning(f"bad category id")
                    continue
                bb.append({
        ...
        boxes = boxes[:, :4]
        input_shape = np.array([inputs["scale_factor"][0], inputs["scale_factor"][1],
        ...
        for i in range(batch_loop_cnt):
            ...
            for ins in inputs:
                bb = self.postprocess(
                    self.ort_sess.run(
                        None, {
                            k: v for k, v in ins.items() if k in self.input_names})[0], ins, thr)
                res.append(bb)
