...
class TableStructureRecognizer(Recognizer):
    ...
    def blockType(self, b):
        ...
        for p, n in patt:
            if re.search(p, b["text"].strip()):
                return n
        tks = [t for t in self.tokenizer.tokenize(
            b["text"]).split(" ") if len(t) > 1]
        if len(tks) > 3:
            if len(tks) < 12:
                return "Tx"
            else:
    ...
    def construct_table(self, boxes, is_english=False, html=False):
        ...
        return self.__desc_table(cap, hdset,
                                 self.__cal_spans(boxes, rows, cols, tbl,
                                                  False),
                                 is_english)
