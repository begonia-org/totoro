...
def highest_degree(dg):
    ...
    m = {"初中": 0, "高中": 1, "中专": 2, "大专": 3,
        "专升本": 4, "本科": 5, "硕士": 6, "博士": 7, "博士后": 8}
...
def forEdu(cv):
    ...
    for ii, n in enumerate(sorted(cv["education_obj"], key=lambda x: x.get("start_time", "3"))):
        ...
        if n.get("school_name") and isinstance(n["school_name"], str):
            ...
            e["sch_nm_kwd"] = sch[-1]
        fea.append(tokenizer.fine_grained_tokenize(
            tokenizer.tokenize(n.get("school_name", ""))).split(" ")[-1])
    ...
    if edu_end_dt:
        try:
            ...
            if edu_end_dt.strip("\n") == "至今": edu_end_dt = cv.get(
                "updated_at_dt", str(datetime.date.today()))
            y, m, d = getYMD(edu_end_dt)
            cv["work_exp_flt"] = min(int(str(datetime.date.today())[
                                     0:4]) - int(y), cv.get("work_exp_flt", 1000))
        except Exception as e:
    ...
    if cv.get("major_kwd"):
        cv["major_tks"] = tokenizer.tokenize(" ".join(maj))
    if cv.get("school_name_kwd"): cv["school_name_tks"] = tokenizer.tokenize(
        " ".join(sch))
    if cv.get("first_school_name_kwd"):
        cv["first_school_name_tks"] = tokenizer.tokenize(" ".join(fsch))
    if cv.get("first_major_kwd"): cv["first_major_tks"] = tokenizer.tokenize(
        " ".join(fmaj))
...
sorted(cv.get("project_obj", []), key=lambda x: str(x.get("updated_at", "")) if isinstance(x, type({})) else "",
                   reverse=True)):
        if n.get("name"):
    ...
    if pro_nms:
        ...
        cv["project_name_tks"] = tokenizer.tokenize(pro_nms[0])
    if desc:
        cv["pro_desc_ltks"] = tokenizer.tokenize(rmHtmlTag(" ".join(desc)))
        cv["project_desc_ltks"] = tokenizer.tokenize(rmHtmlTag(desc[0]))
...
def forWork(cv):
    ...
    corp_tags = []
    for i, n in enumerate(
sorted(cv.get("work_obj", []), key=lambda x: str(x.get("start_time", "")) if isinstance(x, type({})) else "",
                   reverse=True)):
...
if isinstance(n, type("")):
    ...
    if fea["position_name"]:
        cv["position_name_tks"] = tokenizer.tokenize(fea["position_name"][0])
        cv["position_name_sm_tks"] = tokenizer.fine_grained_tokenize(cv["position_name_tks"])
        cv["pos_nm_tks"] = tokenizer.tokenize(" ".join(fea["position_name"][1:]))

    if fea["industry_name"]:
        cv["industry_name_tks"] = tokenizer.tokenize(fea["industry_name"][0])
        cv["industry_name_sm_tks"] = tokenizer.fine_grained_tokenize(cv["industry_name_tks"])
        cv["indu_nm_tks"] = tokenizer.tokenize(" ".join(fea["industry_name"][1:]))

    if fea["corporation_name"]:
        cv["corporation_name_kwd"] = fea["corporation_name"][0]
        cv["corp_nm_kwd"] = fea["corporation_name"]
        cv["corporation_name_tks"] = tokenizer.tokenize(fea["corporation_name"][0])
        cv["corporation_name_sm_tks"] = tokenizer.fine_grained_tokenize(cv["corporation_name_tks"])
        cv["corp_nm_tks"] = tokenizer.tokenize(" ".join(fea["corporation_name"][1:]))

    if fea["responsibilities"]:
        cv["responsibilities_ltks"] = tokenizer.tokenize(fea["responsibilities"][0])
        cv["resp_ltks"] = tokenizer.tokenize(" ".join(fea["responsibilities"][1:]))
    ...
    if type(cv.get("corporation_id")) == type(1):
...
def parse(cv):
    ...
    for k in keys:
        ...
        if k.find("_obj") > 0:
            try:
                ...
                nms = []
                for n in cv[k]:
                    if type(n) != type({}) or "name" not in n or not n.get("name"):
                        continue
                ...
                if nms:
                    t = k[:-4]
                    cv[f"{t}_kwd"] = nms
                    cv[f"{t}_tks"] = tokenizer.tokenize(" ".join(nms))
            except Exception as e:
        ...
        # tokenize fields
        if k in tks_fld:
            cv[f"{k}_tks"] = tokenizer.tokenize(cv[k])
            if k in small_tks_fld:
                cv[f"{k}_sm_tks"] = tokenizer.tokenize(cv[f"{k}_tks"])
    ...
    if cv.get("name"):
        ...
        cv["name_tks"] = (
            tokenizer.tokenize(name) + " " + (" ".join(list(name)) if not re.match(r"[a-zA-Z ]+$", name) else "")
        ) if name else ""
    else:
        cv["integerity_flt"] /= 2.
    ...
    if cv.get("responsibilities"):
        cv["responsibilities_ltks"] = tokenizer.tokenize(rmHtmlTag(cv["responsibilities"]))
    ...
    for k in keys:
        if not re.search(r"_(fea|tks|nst|dt|int|flt|ltks|kwd|id)$", k):
            del cv[k]
    for k in cv.keys():
        if not re.search("_(kwd|id)$", k) or type(cv[k]) != type([]):
            continue
